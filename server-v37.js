...
const INTERNAL_PORT=PUBLIC_PORT+17;
...
const STORE=[
  ...
  {id:"xp_25k",icon:"💫",badge:"LVL 20+",title:"25 000 XP",description:"Пакет опыта для развивающихся предпринимателей.",stars:299,minLevel:20,tier:"xp"},
  {id:"xp_100k",icon:"🔮",badge:"LVL 50+",title:"100 000 XP",description:"Крупный пакет опыта для высоких уровней.",stars:799,minLevel:50,tier:"xp"},

  {id:"income_2h",icon:"📊",badge:"LVL 25+",title:"Инвест-пакет · 2 часа",description:"Сразу получить сумму, равную 2 часам текущего пассивного дохода.",stars:129,minLevel:25,tier:"high"},
  {id:"income_8h",icon:"📈",badge:"LVL 50+",title:"Инвест-пакет · 8 часов",description:"Сразу получить сумму, равную 8 часам текущего пассивного дохода.",stars:299,minLevel:50,tier:"high"},
  {id:"income_24h",icon:"🏙️",badge:"LVL 100+",title:"Инвест-пакет · 24 часа",description:"Сразу получить сумму, равную суткам текущего пассивного дохода.",stars:649,minLevel:100,tier:"high"},
  {id:"income_72h",icon:"🌐",badge:"LVL 150+",title:"Инвест-пакет · 72 часа",description:"Сразу получить сумму, равную трём суткам текущего пассивного дохода.",stars:1499,minLevel:150,tier:"elite"},
  {id:"vip_90d",icon:"💎",badge:"LVL 50+",title:"VIP на 90 дней",description:"+20% к пассивному доходу на 90 дней. Срок складывается с активным VIP.",stars:399,minLevel:50,tier:"high"},
  {id:"vip_365d",icon:"👑",badge:"LVL 100+",title:"VIP на 365 дней",description:"Год VIP: +20% к пассивному доходу. Срок складывается с активным VIP.",stars:1299,minLevel:100,tier:"elite"}
];
...
function verifyInitData(initData){
  ...
  const secret=crypto.createHmac("sha256","WebAppData").update(BOT_TOKEN).digest();const calc=crypto.createHmac("sha256",secret).update(check).digest("hex");
...
function buildXpChallenge(level){
  const scale=Math.max(1,Math.min(250,Number(level||1)));
  const base=900+scale*110;
  const options=[];const seen=new Set();
  while(options.length<4){
    const revenue=rnd(base,base*3),cost=rnd(Math.floor(revenue*.38),Math.floor(revenue*.82)),profit=revenue-cost;
    if(seen.has(profit))continue;seen.add(profit);options.push({revenue,cost,profit});
  }
  let correctIndex=0;for(let i=1;i<options.length;i++)if(options[i].profit>options[correctIndex].profit)correctIndex=i;
  const rewardXp=Math.min(320,24+Math.round(scale*1.35));
  return {correctIndex,rewardXp,options:options.map(({revenue,cost})=>({revenue,cost}))};
}

app.get("/health",(_req,res)=>res.json({ok:true,app:"business-zero-v3.7",economy:"atomic",anticheat:true,xpGame:true,energySeconds:20}));

app.post("/api/xp-game/start",auth,async(req,res)=>{
  try{
    await rpc("guard_player_action_v34",{p_telegram_id:req.tgUser.id,p_action:"xp_game_start"});
    const gate=await rpc("xp_game_start_gate_v37",{p_telegram_id:req.tgUser.id});
    ...
    const q=buildXpChallenge(Number(p.level||1)),sessionId=crypto.randomUUID(),expiresAt=new Date(Date.now()+20000).toISOString();
    await sb("xp_game_sessions",{method:"POST",body:{session_id:sessionId,telegram_id:req.tgUser.id,correct_index:q.correctIndex,reward_xp:q.rewardXp,expires_at:expiresAt}});
    res.json({sessionId,expiresAt,rewardXp:q.rewardXp,question:"У какой сделки чистая прибыль выше?",options:q.options,stats:gate});
  }catch(e){res.status(400).json({error:friendly(e)});}
});

app.post("/api/xp-game/answer",auth,async(req,res)=>{
  try{
    await rpc("guard_player_action_v34",{p_telegram_id:req.tgUser.id,p_action:"xp_game_answer"});
    const result=await rpc("claim_xp_game_v37",{p_telegram_id:req.tgUser.id,p_session_id:String(req.body?.sessionId||""),p_selected_index:Number(req.body?.selectedIndex)});
    res.json({result});
  }catch(e){res.status(400).json({error:friendly(e)});}
});
...
app.get("/api/store",auth,async(req,res)=>{
  try{
    ...
    res.json({products:STORE.map(x=>({...x,locked:Number(p.level)<Number(x.minLevel)})),vipActive:Boolean(p.vip_until&&new Date(p.vip_until)>new Date()),vipUntil:p.vip_until||null,supporterTier:Number(p.supporter_tier||0),playerLevel:Number(p.level||1),purchases:orders||[]});
...
app.post("/api/store/invoice",auth,async(req,res)=>{
  try{
    ...
    await sb(`star_orders?order_id=eq.${encodeURIComponent(orderId)}`,{method:"PATCH",body:{invoice_url:invoiceUrl}});res.json({orderId,invoiceUrl,product});
...
app.listen(PUBLIC_PORT,()=>console.log(`Business Zero v3.7 gateway on :${PUBLIC_PORT} -> ${INTERNAL_PORT}`));
