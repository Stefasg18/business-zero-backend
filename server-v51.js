...
app.get("/api/v51/order/:orderId",auth,async(req,res)=>{try{const orderId=String(req.params.orderId||"");const o=(await sb(`v51_card_orders?order_id=eq.${encodeURIComponent(orderId)}&payer_id=eq.${encodeURIComponent(req.tgUser.id)}&select=order_id,pack_id,stars,status,paid_at&limit=1`))?.[0];if(!o)return res.status(404).json({error:"Заказ не найден"});let cards=[];if(o.status==="paid"){const opening=(await sb(`card_pack_openings?telegram_id=eq.${encodeURIComponent(req.tgUser.id)}&source=eq.${encodeURIComponent(`stars:${orderId}`)}&select=cards&order=opened_at.desc&limit=1`))?.[0];cards=opening?.cards||[];}res.json({order:o,cards});}catch(e){res.status(400).json({error:friendly(e)});}});
